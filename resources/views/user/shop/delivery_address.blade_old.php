...
$details = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" . $Shop->latitude . "," . $Shop->longitude . "&destinations=" . 
$address->latitude . "," . $address->longitude . "&mode=driving&sensor=false&key=AIzaSyAuxmmUPDIXgiw84E9AX7bbbdFzkd0xd50";
$kilometer='';
$json = curl($details);
$details = json_decode($json, TRUE);
$meter = $details['rows'][0]['elements'][0]['distance']['value'];
$kilometer = round($meter / 1000);
echo "distance ".$kilometer;?>

<input type="hidden" name="distance" class="distance{{$address->id}}" id="distance{{$address->id}}" value="<?php echo $kilometer;?>">
                                        ...
                                        <div class="col-md-5 col-sm-5 col-xs-12">
                                            <div class="payment-content-left">
                                                <ul class="nav nav-tabs payment-tabs" role="tablist">
                                                 ...
                                                 <!--  <li class="active">
                                                        <a href="#cash" aria-controls="card" role="tab" data-toggle="tab" class="payment_mode_type cash" data-id="cash" ><span><i class="mdi mdi-credit-card"></i></span> Cash</a>
                                                    </li> --> 
                                ...
                                <table class="table table-responsive">
                                    ...
                                    <tfoot>
                                        ...
                                        <tr>
                                            ...
                                            <td></td>
                                            <td class="text-right updated_delivery_charge "><span>{{currencydecimal(Setting::get('delivery_charge'))}}</span></td>
                                        </tr>
 <?php 
$checked= "true";
$need_to_pay  = 'true';
if($user_wallet_balance > $net)
{ 
    $need_to_pay = 'false';
    $checked = "false";
}
?>                                       <tr>
                                            <th>To Pay</th>
                                            <th></th>
                                            <th class="text-right to_pay"><span>{{currencydecimal(roundPrice($net))}}</span></th>
...
<input type="hidden" id="need_to_pay " name="need_to_pay " value="<?php echo $need_to_pay;  ?>" />
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
...
<input type="checkbox" disabled="<?php echo $checked ; ?>" name="wallet_bal" id="wallet_bal" value=""> Wallet (<?php echo $user_wallet_balance; ?>)
...
$('.payment_mode_type').on('click',function(){
    ...
    var need_to_pay = $('#need_to_pay').val();
    ...
    if(ctype=='card'){
        if(addrs!=''){
          if(need_to_pay){
            ...
            $(this).prop('checked',false);
            $('.btn_checkout').prop('disabled',true);$('#t otal_payment_mode').val("{{Setting::get('payment_mode')}}");
                 $('.btn_checkout').prop('disabled',false);
                 $('.chkouterrors').hide();
                 $('.chkouterrors').html('');
           }
...
$(document).on('click','.chkaddon',function(){
  ...
  $('.update_addr').on('click',function(){
   ...
   var id_name = ".distance"+this_id;
   var total_distance = $(id_name).val();
if(total_distance > 3){
var remain = total_distance ;
} else
{var remain = 3;}
$('.updated_delivery_charge span').html("CAD"+remain );
   var total_amount_pay_new = $(total_amount_pay).val();
  var update_pay = parseInt(total_amount_pay_new) + parseInt(remain); 
$('.to_pay span').html("CAD"+update_pay );
$(total_amount_pay).val() = update_pay ;
});
